...
public class RssFetcher {
    ...
    public List<NewsArticle> fetch(RssSource source, NewsCategory category) {
        log.debug("[RSS] Fetching [{}]: {}", source.getName(), source.getUrl());
        try {
            ...
            List<NewsArticle> articles = feed.getEntries().stream()
                    ...
                    .toList();
            log.debug("[RSS] Fetched {} articles from [{}]", articles.size(), source.getName());
            return articles;
        } catch (Exception e) {
            log.warn("[RSS] Failed to fetch [{}]: {}", source.getName(), e.getMessage());
